...
let homeCache: { data: unknown; expiresAt: number } | null = null;
...
async function buildHomeResponse() {
  ...
  return { heroes: trendingMovies.slice(0, 5), rows };
}

// Pre-warm the home cache on startup
if (tmdb.isConfigured()) {
  buildHomeResponse().then(data => {
...
export const v2Routes: FastifyPluginAsync = async (app) => {
  ...
  app.get('/home', async () => {
    ...
    const now = Date.now();
    if (homeCache && now < homeCache.expiresAt) return homeCache.data;

    const data = await buildHomeResponse();
    homeCache = { data, expiresAt: now + HOME_CACHE_TTL };
    return data;
  });
